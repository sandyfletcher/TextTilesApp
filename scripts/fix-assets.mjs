...
const files = fs.readdirSync(assetsPath).filter(f => { // Read all files in assets folder
  const fullPath = path.join(assetsPath, f);
  return fs.statSync(fullPath).isFile();
});
console.log(`Found ${files.length} asset files`);
const knownAssets = { // known hashes from error messages
...
files.forEach(file => { // Copy files based on known hashes
  if (knownAssets[file]) {
...
const gitignorePath = path.join(distPath, '.gitignore'); // create .gitignore inside dist that allows everything
fs.writeFileSync(gitignorePath, '# Allow all files in dist folder\n!*\n');
console.log('Created permissive .gitignore in dist folder, asset fix complete!');